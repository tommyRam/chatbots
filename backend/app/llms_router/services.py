...
from .schemas import ChatMessageResponse, DocumentSchema
...
def get_relevant_documents_and_formatted_relevant_documents(query: str, chat_id: str) -> Tuple[List[DocumentSchema], List[Tuple[Document, float]]]:
    """Get relevant documents from vectorestore then format is as a List of DocumentSchema"""
    try:
        vectorestore = get_vectorestore_from_namespace(embedding=embedding, namespace=chat_id)
        relevant_docs = vectorestore.similarity_search_with_score(query=query)
        relevant_docs_content_formatted = format_docs_list(relevant_docs)
        
        return (relevant_docs_content_formatted, relevant_docs)
    except Exception as e:
        print(f"Error during retrieving relevant documents: {e}")
        raise e

def simple_RAG_stream_service(query: str, relevant_docs: List[Tuple[Document, float]]):
    """Stream response generated by AI"""
    try:
        ...
        for chunk in rag_chain.stream({
            "context": formatted_relevant_docs_into_one_long_string,
            "question": query
        }):
            yield chunk
    except Exception as e:
        print(f"An error occurred in a simple_RAG_stream_service: {e}")
        raise e
