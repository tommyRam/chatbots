...
async def add_user(user: UserModel, db: orm.Session):
    db.add(user)
    db.commit()
    db.refresh(user)

async def add_refresh_token(refresh_token: RefreshTokenModel, db: orm.Session):
    db.add(refresh_token)
    db.commit()

async def get_refresh_token_by_token(refresh_token: str, db: orm.Session):
...
async def get_active_refresh_token_from_user_id(user_id: str, db: orm.Session):
    return db.query(RefreshTokenModel).filter(
